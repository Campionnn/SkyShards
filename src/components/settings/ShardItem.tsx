...
export const ShardItem: React.FC<ShardItemProps> = React.memo(
  ({ shard, title, description, detailed, rate, defaultRate, onRateChange, onCardClick }) => {
    ...
    return (
      <div
        className="bg-white/5 border border-white/10 rounded-md p-3 hover:bg-white/10 transition-colors duration-200 h-full"
        onClick={onCardClick}
        ...
        <div className="flex items-center justify-between gap-2 h-full">
          <div className="flex-1 min-w-0 self-start">
            <div className="flex items-center gap-1 mb-1">
              ...
              {shard.isDirect ? (
                <span className="px-1.5 py-0.5 text-xs bg-green-500/20 text-green-400 border border-green-500/30 rounded-md flex-shrink-0">
                  Direct
                </span>
              ) : (
                <span className="px-1.5 py-0.5 text-xs bg-fuchsia-500/20 text-fuchsia-400 border border-fuchsia-500/30 rounded-md flex-shrink-0">
                  Fuse
                </span>
              )}
              {isChanged && (
                <span className="px-1.5 py-0.5 text-xs bg-yellow-400/20 text-yellow-300 border border-yellow-400/30 rounded-md flex-shrink-0">
                  Changed
                </span>
              )}
            </div>
            ...
            {detailed && (
              <div className="mt-0.5 text-xs text-slate-400">
                <div className="font-medium text-yellow-500 gap-1 flex items-center truncate">
                  {title}
