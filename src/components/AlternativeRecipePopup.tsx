                    ...
                    return (
                      <React.Fragment key={inputId}>
                        ...
                        <div className="flex items-center gap-1">
                          <img
                            src={`${import.meta.env.BASE_URL}shardIcons/${inputId}.png`}
                            alt={inputShard.name}
                            className="w-4 h-4 object-contain"
                            loading="lazy"
                          />
                          <span className={getRarityColor(inputShard.rarity)}>
                            {inputShard.name}
                          </span>
                        </div>
                      </React.Fragment>
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
          {loading ? (
            ...
            <div className="text-center py-8 text-slate-400">
              No alternatives available for this shard.
            </div>
          ) : (
            <div className="space-y-3">
              {alternatives.map((option, index) => {
                // Create a unique key that includes recipe inputs and output to handle duplicates
                const uniqueKey = option.recipe 
                  ? `${option.recipe.inputs.join('-')}-${option.recipe.outputQuantity}-${index}`
                  : `direct-${index}`;
