...
export const RecipeOverrideManager: React.FC<RecipeOverrideManagerProps> = ({
  ...
  const showAlternatives = useCallback(async (shardId: string, context: AlternativeSelectionContext) => {
    setPopupState({
      isOpen: true,
      alternatives: [],
      shardId,
    ...
    try {
      ...
      const alternatives = await calculationService.getAlternativesForTreeNode(
        shardId,
        params,
        context
      );
  ...
  const handleRecipeSelect = useCallback(async (selectedRecipe: Recipe | null) => {
    if (!popupState.shardId) return;

    try {
      const calculationService = CalculationService.getInstance();
      
      const newResult = await calculationService.applyRecipeOverride(
        popupState.shardId,
        selectedRecipe,
        targetShard,
        requiredQuantity,
